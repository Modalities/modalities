...
GPT2_NORM = GPT2_HIDDEN_DIM * 25  # 25 = num_layer_norms
GPT2_BIAS = 89856
GPT2_LINEAR_NO_PROJECTION = GPT2_ALL - GPT2_LINEAR_PROJECTION - GPT2_EMBEDDING - GPT2_NORM - GPT2_BIAS  # 40107264
...
def test_initialization_groups(model_name, initialization, groups):
    ...
    with CudaEnv(process_group_backend=ProcessGroupBackendType.nccl):
        ...
        if initialization == "scaled":
            ...
            init["bias"] = torch.cat(
                [torch.flatten(parameter.detach()) for name, parameter in params.items() if name.endswith(".bias")]
            )
            for key in ["weight-normal", "weight-projection", "weight-norm", "bias"]:
                # check number of parameters in each group
                assert len(init[key]) == groups[key], f"len(init[{key}]) = {len(init[key])} should be {groups[key]}"

                # check mean and std for each group
                avg = torch.mean(init[key])
                std = torch.std(init[key])
                avg_test = torch.tensor(GPT2_AVG[key], device=avg.device, dtype=avg.dtype)
                std_test = torch.tensor(GPT2_STD[key], device=std.device, dtype=std.dtype)
