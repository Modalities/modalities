...
def config_file_path() -> Path:
    config_file_path = _ROOT_DIR / Path("tests/models/lora/test_configs/" + "simple_gpt2.yaml")
...
@pytest.fixture()
def gpt2_model(set_env, config_dict: dict) -> NNModel:
...
def compute_trainable_num_parameters(model: nn.Module) -> float:
...
def test_convert_linear_layer(gpt2_model: NNModel, r: int, alpha: int):
    layer_to_convert = gpt2_model.lm_head
    assert isinstance(layer_to_convert, nn.Linear)
    lora_linear = convert_layer(layer=layer_to_convert, r=r, alpha=alpha)
...
def test_convert_embedding_layer(gpt2_model: NNModel, r: int, alpha: int):
    layer_to_convert = gpt2_model.transformer.wte
    assert isinstance(layer_to_convert, nn.Embedding)
    lora_embedding = convert_layer(layer=layer_to_convert, r=r, alpha=alpha)
...
def test_conv1d(r: int, alpha: int):
    conv1d = nn.Conv1d(in_channels=3, out_channels=6, kernel_size=5)
    lora_conv1d = convert_convXd(convXd_layer=conv1d, r=r, alpha=alpha)
    assert isinstance(lora_conv1d, LoRAConv1d)
    assert lora_conv1d.conv.weight.shape == conv1d.weight.shape


def test_conv2d(r: int, alpha: int):
    conv2d = nn.Conv2d(in_channels=3, out_channels=6, kernel_size=5)
    lora_conv2d = convert_convXd(convXd_layer=conv2d, r=r, alpha=alpha)
    assert isinstance(lora_conv2d, LoRAConv2d)
    assert lora_conv2d.conv.weight.shape == conv2d.weight.shape


def test_conv3d(r: int, alpha: int):
    conv3d = nn.Conv3d(in_channels=3, out_channels=6, kernel_size=5)
    lora_conv3d = convert_convXd(convXd_layer=conv3d, r=r, alpha=alpha)
    assert isinstance(lora_conv3d, LoRAConv3d)
    assert lora_conv3d.conv.weight.shape == conv3d.weight.shape


def test_incorrect_type(r: int, alpha: int):
    try:
        convert_convXd(convXd_layer=42, r=r, alpha=alpha)
        assert False, "Expected TypeError"
    except TypeError:
        pass


def test_attribute_copying(r: int, alpha: int):
    conv2d = nn.Conv2d(
        in_channels=3, out_channels=6, kernel_size=5, stride=2, padding=1, dilation=2, groups=1, bias=True
    )
...
def test_layer_conversion(gpt2_model: NNModel, r: int, alpha: int, list_allowed_conversion_layers: list):
    recursive_layer_conversion(module=gpt2_model, r=r, alpha=alpha, target_layers=list_allowed_conversion_layers)
    assert not list_allowed_conversion_layers[0] in [
        type(i).__name__ for i in gpt2_model.modules()
    ], "The layer type shouldn't be present in the whole model anymore."


def test_no_layer_conversion(gpt2_model: NNModel, r: int, alpha: int):
    list_allowed_conversion_types = []
    model_before = copy.deepcopy(gpt2_model)
    recursive_layer_conversion(module=gpt2_model, r=r, alpha=alpha, target_layers=list_allowed_conversion_types)
    model_after = gpt2_model
