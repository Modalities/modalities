...
def _load_gpt2(initialization_type: str, std: float | str, use_weight_tying: bool) -> FSDP:
    ...
    config_dict["model_raw"]["config"]["use_weight_tying"] = use_weight_tying
...
def _load_model(
    model_name: str, use_weight_tying: bool, initialization: str = "plain", std: float | str = 0.02
) -> FSDP:
    """load gpt2 or coca model from config and fsdp-wrap it"""
    if model_name == "gpt2":
        model = _load_gpt2(initialization_type=initialization, std=std, use_weight_tying=use_weight_tying)
    elif model_name == "coca":
        if not use_weight_tying:
            raise ValueError(
                "CoCa only supports weight tying. See: "
                "https://github.com/Modalities/modalities/blob/5a22b02b75b228ad92d40915fba0b790b159969d/src/modalities/models/coca/coca_model.py#L170-L172"
            )
...
GPT2_HIDDEN_DIM = 768
GPT2_ALL = 106375680  # in the case of tied weights
GPT2_TOKEN_EMBEDDING = GPT2_HIDDEN_DIM * GPT2_VOCAB_SIZE  # parameters for token embeddings
GPT2_POSITIONAL_EMBEDDING = GPT2_HIDDEN_DIM * GPT2_SEQUENCE_LENGTH  # parameters for positional embeddings
GPT2_EMBEDDING = (
    GPT2_TOKEN_EMBEDDING + GPT2_POSITIONAL_EMBEDDING
...
NR_PARAMETERS = {
    "gpt2": {
        ...
        "untied": {
            "all": GPT2_ALL + GPT2_TOKEN_EMBEDDING,
            "embedding": GPT2_EMBEDDING + GPT2_TOKEN_EMBEDDING,
            "weight-projection": GPT2_WEIGHT_PROJECTION,
            "weight-norm": GPT2_WEIGHT_NORM,
            "weight-normal": GPT2_WEIGHT_NORMAL,
            "bias": GPT2_BIAS,
...
@pytest.mark.parametrize(
    "model_name, use_weight_tying",
    [
        ("gpt2", True),
        ("gpt2", False),
        # Coca only supports weight tying!
        # See:
        # https://github.com/Modalities/modalities/blob/5a22b02b75b228ad92d40915fba0b790b159969d/src/modalities/models/coca/coca_model.py#L170-L172
        ("coca", True),
    ],
)
def test_nr_parameters_per_initialization_group(model_name: str, use_weight_tying: bool):
    """
    ...
    """
    tied_weights_selector = "tied" if use_weight_tying else "untied"

    with CudaEnv(process_group_backend=ProcessGroupBackendType.nccl):
        model = _load_model(model_name, use_weight_tying)
        ...
        for group in INITIALIZATION_GROUPS:
            ...
            assert nr_parameters_group == NR_PARAMETERS[model_name][tied_weights_selector][group], (
                f"nr_parameters for {model_name}/{group} = {nr_parameters_group} "
                + f"should be {NR_PARAMETERS[model_name][tied_weights_selector][group]}"
                + f"Actual - Expected: {nr_parameters_group - NR_PARAMETERS[model_name][tied_weights_selector][group]}"
            )
            nr_parameters_all += nr_parameters_group

        # check total number of parameters
        assert nr_parameters_all == NR_PARAMETERS[model_name][tied_weights_selector]["all"], (
            f"total number of parameters for {model_name} = {nr_parameters_all} "
            + f"should be {NR_PARAMETERS[model_name][tied_weights_selector]['all']}"
        )
...
@pytest.mark.parametrize(
    "model_name, initialization, std, use_weight_tying, success",
    [
        # std = 0.02
        ("gpt2", "plain", 0.02, True, True),
        ("gpt2", "plain", 0.02, False, True),
        ("gpt2", "scaled", 0.02, True, True),
        ("gpt2", "scaled", 0.02, False, True),
        ("gpt2", "scaled_embed", 0.02, True, True),
        ("gpt2", "scaled_embed", 0.02, False, True),
        ("coca", "plain", 0.02, True, True),
        ("coca", "scaled", 0.02, True, False),  # scaled not implemented for coca
        ("coca", "scaled_embed", 0.02, True, False),  # scaled_embed not implemented for coca
        # std = 'auto'
        ("gpt2", "plain", "auto", True, True),
        ("gpt2", "plain", "auto", False, True),
        ("gpt2", "scaled", "auto", True, True),
        ("gpt2", "scaled", "auto", False, True),
        ("gpt2", "scaled_embed", "auto", True, True),
        ("coca", "plain", "auto", True, False),  # auto not implemented for coca
        ("coca", "scaled", "auto", True, False),  # auto not implemented for coca
        ("coca", "scaled_embed", "auto", True, False),  # scaled_embed not implemented for coca
    ],
)
def test_statistical_distribution_for_each_initialization_group(
    model_name: str, initialization: str, std: float | str, use_weight_tying: bool, success: bool
):
    """
