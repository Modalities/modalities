...
@pytest.mark.parametrize(
    "num_ranks,global_num_tokens,sequence_length,expected",
    [(2, 10, 2, 2), (2, 11, 2, 2), (2, 12, 2, 3)],
)
def test_get_local_num_batches_from_num_tokens(
    num_ranks: int, global_num_tokens: int, sequence_length: int, expected: int
):
    assert (
        NumberConversion.get_local_num_batches_from_num_tokens(num_ranks, global_num_tokens, sequence_length)
        == expected
    )
