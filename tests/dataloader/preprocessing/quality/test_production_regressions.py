...
def test_skip_existing_rejects_a_truncated_packed_file(tmp_path: Path):
    ...
    assert not pack_many._is_usable(absent)
    ...
    assert not pack_many._is_usable(truncated), "a zero-length data section is not a finished pack"

    garbage = tmp_path / "garbage.pbin"
    garbage.write_bytes(b"not a pbin")
    assert not pack_many._is_usable(garbage)
...
def test_a_healthy_packed_file_is_skipped(tmp_path: Path):
    ...
    assert pack_many._is_usable(healthy)
