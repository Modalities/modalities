...
# NOTE: We need to run the tests in a torch distributed environment with at eight GPUs.
# CUDA_VISIBLE_DEVICES=0,1,2,4,5,6,7 torchrun --rdzv-endpoint localhost:29502 --nnodes 1 --nproc_per_node 8 \
...
@pytest.mark.skipif(
    "RANK" not in os.environ or torch.cuda.device_count() < 8,
    reason="This e2e test requires 8 GPUs and a torchrun distributed environment.",
)
class TestWarmstart:
    @staticmethod
    ...
    def test_warm_start(self):
        ...
        # To do so, we have two configs. The first config trains a model for 8 steps and
        ...
        try:
            ...
            gpt2_warm_start_after_4_steps_config_file_path = working_dir / "gpt2_warm_start_from_step_4_pp_tp.yaml"
            gpt2_warm_start_after_4_steps_dict = load_app_config_dict(
                gpt2_warm_start_after_4_steps_config_file_path, experiment_id="1"
            )

            # adopt the checkpoint path
            gpt2_warm_start_after_4_steps_dict["app_state"]["config"]["checkpoint_dir_path"] = (
                checkpoint_path + "/0/eid_0-seen_steps_4-seen_tokens_4096-target_steps_7-target_tokens_7168"
            )
            ...
            with CudaEnv(process_group_backend=ProcessGroupBackendType.nccl):
                try:
                    ...
                    print(
                        main_obj_0.config_dict["settings"]["training_target"]["num_target_tokens"]["config"][
                            "dataset_path"
                        ]
                    )
                    components_0 = main_obj_0.build_components(
                    ...
                    if dist.get_rank() == 0:
                        messages_0: list[
                            Message[EvaluationResultBatch]
                        ] = components_0.evaluation_subscriber.message_list
                        ...
                        assert Path(checkpoint_info["checkpoint_folder_path"]).exists()

                        checkpoint_paths = list(Path(checkpoint_path).glob("**/*seen_steps_*-seen_tokens_*"))
                        ...
                        cp_info_model_seen_steps, cp_info_model_seen_tokens = extract_seen_steps_and_tokens(
                            checkpoint_info["checkpoint_folder_path"]
                        )

                        assert cp_info_model_seen_steps == max_seen_steps
                        assert cp_info_model_seen_tokens == max_seen_tokens
                    ...
                    components_1 = main_obj_1.build_components(
                        components_model_type=TrainingComponentsInstantiationModel
                    )
                    ...
                    if dist.get_rank() == 0:
                        messages_1: list[
                            Message[EvaluationResultBatch]
                        ] = components_1.evaluation_subscriber.message_list
                    ...
                    assert (
                        components_1.app_state.lr_scheduler.last_epoch == components_0.app_state.lr_scheduler.last_epoch
                    )
                    assert (
                    ...
                    logging.error(f"Exception in rank {os.environ.get('RANK', -1)}: {e}")
                    logging.error(tb)
                    raise
        finally:
            logging.info(f"Rank {os.environ.get('RANK', -1)} cleaning up.")
            if int(os.environ.get("RANK", -1)) == 0:
                try:
                    if tmp_folder.exists():
                        shutil.rmtree(tmp_folder)
                except Exception as e:
                    logging.warning(f"Rank {os.environ.get('RANK', -1)}: failed to remove tmp folder {tmp_folder}: {e}")
