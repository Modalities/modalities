...
class TestFSDP1:
    @staticmethod
    def _test_fsdp1_loss_convergence_thread(process_id: int, world_size: int, config_file_path: Path):
        components = run_modalities_training(
            process_id=process_id, world_size=world_size, config_file_path=config_file_path
        )
    ...
    @staticmethod
    def test_fsdp1_loss_convergence():
        working_dir = Path(os.path.dirname(__file__))
        config_file_path = working_dir / "configs/fsdp1_gpt2_train_num_steps_8.yaml"
        world_size = 2
        mp.spawn(
            TestFSDP1._test_fsdp1_loss_convergence_thread,
