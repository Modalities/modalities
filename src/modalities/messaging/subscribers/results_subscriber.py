...
class WandBEvaluationResultSubscriber(MessageSubscriberIF[EvaluationResult]):
    ...
    def _consum_evaluation_results_batch(self, message: Message[EvaluationResult]):
        ...
        trackables = {
            f"{eval_result.dataloader_tag} {trackable_key}": trackable_values
            for trackable_key, trackable_values in eval_result.trackables.items()
        }

        wandb.log(data=trackables, step=eval_result.train_step_id + 1)
