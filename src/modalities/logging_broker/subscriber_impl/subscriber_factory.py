...
class ProgressSubscriberFactory:
    ...
    def get_rich_progress_subscriber(
        ...
        skip_num_local_train_batches: int,
        world_size: int,
        global_num_seen_samples: int,
        local_rank: int,
    ) -> RichProgressSubscriber:
        if local_rank == 0:
            skip_num_local_train_batches = global_num_seen_samples // world_size // train_dataloader.sampler_batch_size

            train_split_num_samples = {
                train_dataloader_tag: (len(train_dataloader) + skip_num_local_train_batches)
                * world_size
                * train_dataloader.sampler_batch_size
            }

            eval_splits_num_samples = {
                dataloader.dataloader_tag: len(dataloader) * world_size * dataloader.sampler_batch_size
...
class ResultsSubscriberFactory:
    ...
    def get_wandb_result_subscriber(
        ...
        if mode == WandbMode.ONLINE or mode == WandbMode.OFFLINE:
            result_subscriber = WandBEvaluationResultSubscriber(
                world_size, project, experiment_id, mode, directory, experiment_config
            )
        else:
