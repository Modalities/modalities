...
def get_experiment_id_from_config(config_file_path: Optional[Path], hash_length: Optional[int] = 16) -> str:
...
def get_synced_string(string_to_be_synced: str, from_rank: int = 0, max_string_byte_length: int = 1024) -> str:
...
def get_synced_experiment_id_of_run(
    ...
    hash_length: Optional[int] = 16,
    max_experiment_id_byte_length: int = 1024,
) -> str:
...
def get_total_number_of_trainable_parameters(model: FSDPX, device_mesh: DeviceMesh | None) -> Number:
    ...
    if isinstance(model, FSDP1):
        ...
        if model.sharding_strategy.name == "NO_SHARD":
            ...
            raise ValueError(f"Unknown sharding strategy: {model.sharding_strategy.name}")
        ...
        #
        # >>> parameter_tensor = list(model.parameters())[0]
        #
        # >>> type(parameter_tensor)
        # <class 'torch.distributed.tensor.DTensor'>
        #
        # >>> parameter_tensor.shape
        # torch.Size([50304, 128])
        #
        # >>> parameter_tensor.to_local().shape
        # torch.Size([25152, 128])
        #
        # >>> parameter_tensor.numel()
        # 6438912
        #
