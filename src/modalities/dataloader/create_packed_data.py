...
class PackedDataGenerator:
    ...
    def _current_offset(self) -> int:
        return (
            self._total_data_bytes
            + type(self).DATA_HEAD_SIZE_IN_BYTES
            + type(self).CODECS_HEAD_SIZE_IN_BYTES
        )
    ...
    def run(self, dst_path: Path = None):
        ...
        with dst_path.open("wb") as f:
            ...
            codecs_bytes = pickle.dumps(
                [type(codec).__name__ for codec in self.codecs]
            )

            # allocate bytes for data header and write codecs header
            f.write((0).to_bytes(type(self).DATA_HEAD_SIZE_IN_BYTES, byteorder="big"))
            f.write(
                len(codecs_bytes).to_bytes(
                    type(self).DATA_HEAD_SIZE_IN_BYTES, byteorder="big"
                )
            )
    ...
    def _update_data_length_in_pre_allocated_header(self, dst_path: Path):
        ...
        m = np.memmap(
            ...
            mode="r+",
            offset=0,
            shape=(type(self).DATA_HEAD_SIZE_IN_BYTES,)
        )
        m[:] = header_content[:]
    ...
    def _process_line(self, f: IO, line: str):
        ...
        # exceeds size limit
        if (
            (self._max_data_bytes is not None) and
            (self._total_data_bytes >= self._max_data_bytes)
        ):
            raise StopIteration
