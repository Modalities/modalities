...
def fsdp_main(args):
    ...
    mixed_precision_policy, t5_auto_wrap_policy = get_policies(
        train_config, rank)
    ...
    # tp_rank = dist.get_group_rank(tp_pg)
    # tp_rank = local_rank % tp_size
    print("2d mesh ===========", twod_mesh)
    ...
    for epoch in range(1, args.epochs + 1):
        t0 = time.time()
        train_accuracy = train(args, model, rank, world_size,
                               train_loader, optimizer, epoch, sampler=sampler1)
    ...
    if rank == 0:
        ...
        print(
            f"train accuracy {train_acc_tracking} and validation accuracy {val_acc_tracking}")
        print(
            f"train time per epoch is {(training_end_time-training_start_time/ args.epochs)}")
        print(f"allocated memory {mem_alloc_tracker}")
        print(f"reserved memory  {mem_reserved_tracker}")
