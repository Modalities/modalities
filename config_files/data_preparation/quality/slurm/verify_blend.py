...
for rec in manifest["datasets"]:
    ...
    for p in pbins:
        ...
        del s
    est, sel = rec["est_tokens_kept"], rec["n_documents_kept"]
    err = (ntok - est) / est if est else 0.0
    tot_est += est; tot_pack += ntok
    flags = ""
    if abs(err) > 0.05: flags += "  TOKENS>5%"; problems.append(f"{name}: tokens off {err:+.2%}")
    if ndoc != sel: flags += "  DOCS MISMATCH"; problems.append(f"{name}: {sel:,} selected vs {ndoc:,} packed")
    if len(pbins) != len(rec["index_files"]): flags += "  FILE COUNT"; problems.append(f"{name}: {len(rec['index_files'])} idx vs {len(pbins)} pbin")
    print(f"{name:<16} {est:>16,} {ntok:>16,} {err*100:>6.2f}% {sel:>14,} {ndoc:>14,}{flags}", flush=True)
print("-" * 92)
...
owned = [p for p in stray if Path(p).owner() == "richard.rutmann"] if stray else []
print(f"  non-jsonl files: {len(stray)} (pre-existing README/.gitattributes)")
print(f"  files owned by us: {len(owned)}")
if owned: problems.append(f"{len(owned)} files written into the source tree")
print()
